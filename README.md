...
5. **Retrieval** (`vector_db.py`): top-100 nearest-neighbor search via Qdrant
...
- **Retrieval**: top-100 nearest-neighbor candidates from Qdrant, then cross-encoder reranking (ms-marco-MiniLM-L-6-v2) to surface the top 3-5 most relevant chunks.
- **Generation**: local LLM via Ollama (qwen3:1.7b). The top-ranked chunk is passed as context to generate a grounded answer.
